...
### Changed
- **Web `/today` now renders the canonical artifact (#174).** The morning gist is rendered to the Fraunces/IBM Plex broadsheet HTML at generation time and stored on the gist doc as `renderedHtml`. `/today` renders that exact artifact in an `<iframe>` instead of re-deriving the sections in Angular — so the web view can no longer drift from what was generated, and the missing-on-web sections (masthead, issue number, rhythms) are always present. **Print** prints the iframe, so the browser uses the broadsheet's `@media print` + `@page` rules and produces the properly paginated 2-page newspaper. (A responsive "feels-like-paper" layout for web + email is deferred to a follow-up.)
- **Rhythms "season day" fixed (#174).** `(month % 3) * 30 + date` produced a meaningless number (June read "Day 67"; July 1 jumped backward). Now counts real days since the season began, using astronomical boundaries (summer = June solstice, not Jun 1), so e.g. June 7 reads "Spring, Day 80".
...
### Fixed
- **New users no longer stranded out of the delivery loop when the onboarding preview fails (#195).** The scheduler only picks up users whose `nextDeliveryAt` field exists (Firestore inequality filters exclude docs missing the field). `generateGistOnDemand` seeded `nextDeliveryAt` *after* a successful preview generation — so if that first generation threw or timed out, the field was never written and the user never received a recurring morning gist. `nextDeliveryAt` is now computed up front and seeded even on the failure path (leaving `lastGeneratedDate` unset so the scheduler regenerates today's gist at the user's normal slot as recovery).
