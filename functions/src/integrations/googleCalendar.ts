...
function getOAuthConfig(): {
  ...
  const clientId = getSecretValue(GOOGLE_CLIENT_ID) ?? process.env.GOOGLE_CLIENT_ID ?? null;
  const clientSecret =
    getSecretValue(GOOGLE_CLIENT_SECRET) ?? process.env.GOOGLE_CLIENT_SECRET ?? null;
  return { clientId, clientSecret };
}

async function loadStoredTokens(
  userId: string
): Promise<{ tokens: StoredGoogleTokens | null; location: TokenStorageLocation | null }> {
...
function buildTimeBounds(dateKey: string, timeZone: string): {
  timeMin: string;
  timeMax: string;
} {
...
function cleanNote(location?: string | null, description?: string | null): string | undefined {
  const notes: string[] = [];
...
async function refreshAccessToken(
  ...
  if (!response.ok) {
    ...
    logger.warn('Google OAuth token refresh failed.', { status: response.status, text });
    return null;
  }
...
async function listCalendarEvents(params: {
  ...
  return (await response.json()) as { items: Array<{
    summary?: string | null;
    location?: string | null;
    description?: string | null;
    start?: { date?: string | null; dateTime?: string | null } | null;
  ...
  try {
    ...
    try {
      ...
      if (tokens.refreshToken) {
        const retryTokens = await refreshAccessToken(refreshedTokens, { clientId, clientSecret });
