        ...
        res
          ...
          .send(
            renderPopupResultHtml({
              success: true,
              message:
                'Google Calendar connected. You can close this window now.',
              origin: verifiedState.origin,
            }),
          );
        return;
      } catch (error) {
        logger.error('Google OAuth callback exchange error', { error });
        res
          .status(500)
          .type('html')
          .send(
            renderPopupResultHtml({
              success: false,
              message:
                'Unable to connect Google Calendar right now. Please try again.',
              origin: verifiedState.origin,
            }),
          );
        return;
